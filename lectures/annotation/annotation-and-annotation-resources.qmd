...
- :one: an **annotation resource**: reference information on entities or a
  mapping of one type of identifiers to another.
- :two: a **rule** that defines how to assign labels to entities
- *examples* (maybe through drawings?)
...
- :information_source: many resources are community provided :muscle:.
...
- First time data is downloaded, subsequent calls will load the data from the
  cache :muscle:
...
- *task*: get official HGNC gene symbols (names) the genes.
...
- `multiVals`... to handle ambiguity; default is `"first"` but there are other
  options too.
...
- :dart: going one step further ... annotating genes to biological pathways.
